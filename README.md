...
Secreto para firmar/verificar los tokens JWT
...
Tiempo de expiracion de los tokens (ej: 7d, 1h)
...
Origen permitido para llamadas desde el front-end
...
Crea un usuario nuevo. Body: nombre, email, password, rol (opcional). Devuelve el usuario creado + token.
...
Inicia sesion. Body: email, password. Devuelve el usuario + token.
...
Lista las tareas del usuario. Soporta ?page=&limit=&sortBy=.
...
Lista las tareas vencidas (no completadas) del usuario.
